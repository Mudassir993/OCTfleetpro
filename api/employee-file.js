...
async function auth(req){
  ...
  try{
    const r=await c.query(`SELECT u.id,u.username,u.name,u.role,u.status FROM app_sessions s JOIN app_users u ON u.id=s.user_id WHERE s.token_hash=$1 AND s.expires_at>now() AND u.status='Active'`,[tokenHash(raw)]);
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    let doc;
    try{
      const r=await c.query('SELECT id,doc_key,name,content_type FROM employee_documents WHERE pathname=$1',[pathname]);
      doc=r.rows[0];
    }finally{c.release();}
    ...
    const result=await presignUrl(signed,{pathname,operation:'get',validUntil,access:'private'});

    res.statusCode=302;
    res.setHeader('Location',result.presignedUrl);
    res.setHeader('Cache-Control','private, no-store');
    res.setHeader('Content-Disposition',`attachment; filename*=UTF-8''${encodeURIComponent(doc.name||'document')}`);
    return res.end();
  }catch(e){
    console.error('Employee Blob signed download error',e);
    return json(res,500,{error:e.message||'Could not create secure download URL'});
  }
};
